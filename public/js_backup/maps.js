...
function createMarker(propertyResult) {
    ...
    for (var i = 0; i < propertyResult.length; i++) {
        ...
        marker = new google.maps.Marker({
            ...
            anchorPoint: new google.maps.Point(0, -29),
            position: {
                lat: propertyResult[i].Lat,
                lng: propertyResult[i].Lng
            }
        });
        bounds.extend({
            lat: propertyResult[i].Lat,
        ...
        markers.push(marker);
        var content = propertyResult[i].GeoLocation;
